...
image_list = []
for filename in glob.glob(r'E:\Exdark_split\normal/*'): #assuming gif
    im= cv2.imread(filename)
    im = tf.image.resize(im, [input_img_size[0], input_img_size[1]])
    image_list.append(im)
image_list2 = []
for filename in glob.glob(r'E:\Exdark_split\Single/*'): #assuming gif
    im= cv2.imread(filename)
    im = tf.image.resize(im, [input_img_size[0], input_img_size[1]])
    image_list2.append(im)
image_list3 = []
for filename in glob.glob(r'E:\COCO2017_Test\car/*'): #assuming gif
    im= cv2.imread(filename)
    im = tf.image.resize(im, [input_img_size[0], input_img_size[1]])
...
train_normal = tf.data.Dataset.from_tensor_slices(image_list)
train_dark = tf.data.Dataset.from_tensor_slices(image_list2)
test_normal = tf.data.Dataset.from_tensor_slices(image_list3)
...
train_normal = (
    train_normal.map(preprocess_train_image, num_parallel_calls=autotune)
    .shuffle(buffer_size)
    .batch(batch_size)
)
train_dark = (
    train_dark.map(preprocess_train_image, num_parallel_calls=autotune)
    .shuffle(buffer_size)
    .batch(batch_size)
)
...
test_normal = (
    test_normal.map(preprocess_test_image, num_parallel_calls=autotune)
    .cache()
    .shuffle(buffer_size)
    .batch(batch_size)
)
...
# gen_G = tf.keras.models.load_model(r"C:\Users\rom21\OneDrive\Desktop\Final_project_afeka\code\augmentation\normal2low")
#
...
_, ax = plt.subplots(4, 2, figsize=(10, 15))
for i, samples in enumerate(zip(train_normal.take(4), train_dark.take(4))):
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i, img in enumerate(test_normal.take(self.num_img)):
...
cycle_gan_model.fit(
    tf.data.Dataset.zip((train_normal, train_dark)),
    epochs=1,
...
# Load the checkpoints
cycle_gan_model.gen_G.save('cyclegan_Single.090')
# weight_file = "./saved_checkpoints/cyclegan_checkpoints.090"
# cycle_gan_model.load_weights(weight_file).expect_partial()
# print("Weights loaded successfully")

_, ax = plt.subplots(4, 2, figsize=(10, 15))
for i, img in enumerate(test_normal.take(4)):
    prediction = cycle_gan_model.gen_G(img, training=False)[0].numpy()
    prediction = (prediction * 127.5 + 127.5).astype(np.uint8)
    img = (img[0] * 127.5 + 127.5).numpy().astype(np.uint8)

    ax[i, 0].imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    ax[i, 1].imshow(cv2.cvtColor(prediction, cv2.COLOR_BGR2RGB))
    ax[i, 0].set_title("Input image")
    ax[i, 0].set_title("Input image")
    ax[i, 1].set_title("Translated image")
    ax[i, 0].axis("off")
